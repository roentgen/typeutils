...
int main()
{
...
#if 1
	printf("%zu (5)\n", T::offset_from<0, 2>());
	printf("%zu (5)\n", T::offset_from<0, 2, 1>()); /* sel<agg<double, double>> ÇÃÇŸÇ§ÇæÇ™, sel ÇÃäJénÇ‹Ç≈ÇÃ offset Ç»ÇÃÇ≈ 5 */
	printf("%zu(13)\n", T::offset_from<0, 2, 1, 1>()); /* sel<agg<double, double>> ÇÃìÒÇ¬ñ⁄ÇÃ double Ç‹Ç≈ÇÃ offset Ç»ÇÃÇ≈ 13 */
	printf("%zu (5)\n", T::offset_from<0, 2, 0>()); /* sel<agg<int, int>> Ç≈ìØè„ */
	printf("%zu(21)\n", T::offset_from<0, 3>());    /* sel<agg<int,int>>,agg<double,double>> ÇÃéüÇ»ÇÃÇ≈ 21 */
#endif
	...
	*get<T0, 0, 1, 0, 0>::addr(storage) = 0xdeadbeef;
	...
	/* align=0 ÇÃ type_t ÇÕè„à  enclosure ÇÃ placement align Ç…è]Ç§ */
...
#endif
	/* agg_t ÇÃ alignment ÇÕ type_t Ç…è]Ç§ */
	...
	/* ëOï˚,å„ï˚Ç… padding Ç™ì¸ÇÈ(Ç«ÇøÇÁÇ‡ type_t ÇÃäOÇ… padding Ç™Ç†ÇÈ) */
